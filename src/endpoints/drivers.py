...
from src.command_handlers.drivers_handlers import (
    ...
    handle_toggle_driver_status,
)
from src.endpoints.views.drivers_view import get_all_drivers, get_driver_by_id, get_online_drivers
...
@router.get("/drivers/online/", status_code=200)
async def list_online_drivers(uow: UnitOfWork = Depends(get_uow)):
    drivers = await get_online_drivers(uow)
    return drivers

@router.put("/drivers/{driver_id}/toggle-status", status_code=200)
async def toggle_driver_status(driver_id: int, uow: UnitOfWork = Depends(get_uow)):
    cmd = ToggleDriverStatus(id=driver_id)
    await handle_toggle_driver_status(cmd, uow)
